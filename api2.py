...
class TimetableGenerator:
    def __init__(self, api_key: str, subjects: List[Dict], population_size: int = 50):
    ...
    def generate_timetable(self, section: str) -> pd.DataFrame:
        ...
        # First attempt: Pure GA approach 
        for generation in range(20):
            ...
            self.update_progress(int((generation + 1) / 20 * 100))
            
            if self._is_solution_valid(best_solution):
                return self._convert_to_dataframe(best_solution)
        
        # If GA fails to find a perfect solution, return the best one found
        return self._convert_to_dataframe(best_solution)

    def _initialize_population(self) -> List[List[Session]]:
        """Initialize random population of timetables"""
        return [self._create_single_timetable() for _ in range(self.population_size)]
    ...
    def _get_random_time_slot(self, duration: float) -> TimeSlot:
        ...
        valid_slots = [
            ...
            if (slot < "12:20-13:00" and SLOTS.index(slot) + int(duration * 2) <= len(SLOTS)) or
            slot > "13:00-13:40"
        ]
    ...
    def _convert_to_dataframe(self, solution: List[Session]) -> pd.DataFrame:
        ...
        # Add lunch break
        for day in DAYS:
            df.at[day, "11:00-11:20"] = "LUNCH BREAK"
        ...
        for session in solution:
            ...
            for i in range(duration_slots):
                if start_idx + i < len(SLOTS):
                    ...
                    if slot != "11:00-11:20":  # Skip lunch slot
    ...
    def _ai_repair_solution(self, solution: List[Session]) -> List[Session]:
        """Placeholder for AI-assisted repair - not implemented in this version"""
        return solution
        
    def _generate_with_relaxed_constraints(self, section: str) -> pd.DataFrame:
        """Fallback method with relaxed constraints"""
        if self.fitness_history:
            # Find the best solution from history
            best_index = np.argmax(self.fitness_history)
            if best_index < len(self.fitness_history):
                # Just return the best solution we've found so far
                return self._convert_to_dataframe(self._initialize_population()[0])
        
        # If all else fails, create a simple valid timetable
        return self._convert_to_dataframe(self._create_single_timetable())
    ...
    with tab1:
        ...
        if generate_button:
            try:
                ...
                # Start a separate thread for timetable generation
                import threading
                ...
                # Update progress and plot while waiting
                while not st.session_state.generation_complete and not st.session_state.generation_error:
                    progress_bar.progress(generator.get_progress())
                ...
                if st.session_state.generation_error:
                    results_container.error(f"Error generating timetable: {st.session_state.generation_error}")
                else:
                ...
                st.error(f"Error setting up timetable generation: {str(e)}")
