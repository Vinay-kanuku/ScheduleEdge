        ...
        for subject in SUBJECTS:
            for day in DAYS:
                ...
                for slot in TIME_SLOTS:
                    if slot != "LUNCH":
                        subject_slots.append(self.slots[(day, slot, subject.name)])
                self.model.Add(sum(subject_slots) <= subject.weekly_hours // 2)  # Adjust as per requirement
...
class TimetableGenerator:
    ...
    def generate_timetable(self, section: str) -> pd.DataFrame:
        ...
        # First attempt: Pure GA approach with sequential processing
        for generation in range(20):
            if best_solution is None:
                population = self._initialize_population_sequential()
            else:
    ...
    def _initialize_population_sequential(self) -> List[List[Session]]:
        """Initialize random population of timetables sequentially"""
